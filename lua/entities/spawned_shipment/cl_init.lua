...
function ENT:drawFloatingGun()
    ...
    -- Position the gun
    local gunPos = self:GetAngles():Up() * 20 + ang:Up() * (math.sin(CurTime() * 0) * 8)
    self:GetgunModel():SetPos(pos + gunPos)


    -- Make it dance
    ang:RotateAroundAxis(ang:Up(), (CurTime() * 140) % 360)
...
function ENT:drawInfo()
    ...
    cam.Start3D2D(Pos + Ang:Up() * 13, Ang, 0.12)
        draw.WordBox(2, -TextWidth * 0.5 + 5, -30, text, "HUDNumber5", Color(113, 112, 94, 175), Color(0, 0, 0, 255))
        draw.WordBox(2, -TextWidth2 * 0.5 + 5, 18, contents, "HUDNumber5", Color(113, 112, 94, 175), Color(0, 0, 0, 255))
    cam.End3D2D()
    ...
    cam.Start3D2D(Pos + Ang:Up() * 25, Ang, 0.14)
        draw.WordBox(2, -TextWidth * 0.5 + 5, -50, text, "HUDNumber5", Color(113, 112, 94, 175), Color(0, 0, 0, 255))
        draw.WordBox(2, -TextWidth2 * 0.5 + 0, -2, self:Getcount(), "HUDNumber5", Color(113, 112, 94, 175), Color(0, 0, 0, 255))
    cam.End3D2D()
